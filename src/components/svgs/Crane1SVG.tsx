import type { SVGProps } from 'react';
import { getColor } from './colors';

interface Crane1SVGProps extends SVGProps<SVGSVGElement> {
  accentColor?: string;
  size?: number;
}

export default function Crane1SVG({ accentColor = 'yellow', size = 32, ...props }: Crane1SVGProps) {
  const g500 = getColor('gray', 500);
  const g100 = getColor('gray', 100);
  const ac200 = getColor(accentColor, 200);
  const ac300 = getColor(accentColor, 300);
  const g200 = getColor('gray', 200);
  const g300 = getColor('gray', 300);
  const ac100 = getColor(accentColor, 100);
  const g600 = getColor('gray', 600);

  return (
    <svg
      fill="none"
      width={size}
      height={size}
      viewBox="0 0 512 512"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        fill={g500}
        d="M42.01,270.441c-4.35,0-7.877-3.527-7.877-7.877V157.538c0-4.35,3.527-7.877,7.877-7.877
      	s7.877,3.527,7.877,7.877v105.026C49.887,266.914,46.36,270.441,42.01,270.441z"
      />
      <path
        fill={g100}
        d="M42.01,301.949c-10.134,0-18.379-8.246-18.379-18.379s8.246-18.379,18.379-18.379
      	c4.35,0,7.877,3.527,7.877,7.877s-3.527,7.877-7.877,7.877c-1.447,0-2.626,1.178-2.626,2.626s1.178,2.626,2.626,2.626
      	c4.35,0,7.877,3.527,7.877,7.877S46.36,301.949,42.01,301.949z"
      />
      <path
        fill={ac200}
        d="M430.605,157.538H0c0-29.002,23.511-52.513,52.513-52.513h378.092V157.538z"
      />
      <g>
        <rect x="252.062" y="105.026" fill={ac300} width="178.544" height="52.513" />
        <path
          fill={ac300}
          d="M375.467,294.067v-63.005c0-0.003,0-0.007,0-0.01v-63.01c0-4.35-3.526-7.877-7.877-7.877h-63.015
      		c-4.351,0-7.877,3.527-7.877,7.877v63.01c0,0.002,0,0.003,0,0.005c0,0.002,0,0.003,0,0.005v63.004c0,0.002,0,0.003,0,0.005
      		c0,0.002,0,0.003,0,0.005v63.004c0,0.002,0,0.003,0,0.005s0,0.003,0,0.005v63.012c0,4.35,3.526,7.877,7.877,7.877h63.015
      		c4.351,0,7.877-3.527,7.877-7.877v-63.01c0-0.003,0-0.007,0-0.011v-63.005C375.467,294.074,375.467,294.07,375.467,294.067z
      		 M347.222,262.564l12.491-12.491v24.981L347.222,262.564z M356.45,294.072l-20.368,20.369l-20.368-20.369l20.368-20.368
      		L356.45,294.072z M336.082,251.425l-20.368-20.368l20.368-20.369l20.368,20.368L336.082,251.425z M359.713,212.039l-12.491-12.491
      		l12.491-12.491V212.039z M336.082,188.41l-12.491-12.492h24.981L336.082,188.41z M324.943,199.549l-12.492,12.492v-24.982
      		L324.943,199.549z M324.943,262.564l-12.492,12.492v-24.982L324.943,262.564z M324.943,325.579l-12.492,12.492v-24.982
      		L324.943,325.579z M336.082,336.72l20.368,20.368l-20.368,20.369l-20.368-20.369L336.082,336.72z M324.943,388.595l-12.492,12.492
      		v-24.982L324.943,388.595z M336.082,399.735l12.491,12.491h-24.98L336.082,399.735z M347.222,388.595l12.491-12.491v24.981
      		L347.222,388.595z M347.222,325.579l12.491-12.491v24.981L347.222,325.579z"
        />
      </g>
      <g>
        <polygon
          fill={ac200}
          points="252.062,472.615 420.103,472.615 399.097,420.103 273.067,420.103 	"
        />
        <rect x="283.569" y="136.533" fill={ac200} width="105.026" height="42.01" />
      </g>
      <rect x="420.103" y="84.021" fill={g200} width="84.021" height="105.026" />
      <path
        fill={g300}
        d="M504.123,165.415h-84.021c-4.351,0-7.877-3.527-7.877-7.877s3.526-7.877,7.877-7.877h84.021
      	c4.351,0,7.877,3.527,7.877,7.877S508.474,165.415,504.123,165.415z"
      />
      <polygon
        fill={ac200}
        points="304.574,94.522 325.579,31.508 367.59,31.508 367.59,115.528 304.574,115.528 "
      />
      <path
        fill={ac100}
        d="M231.056,139.159H73.518c-4.35,0-7.877-3.527-7.877-7.877s3.527-7.877,7.877-7.877h157.538
      	c4.35,0,7.877,3.527,7.877,7.877S235.407,139.159,231.056,139.159z"
      />
      <path
        fill={g500}
        d="M441.108,480.492H231.056c-4.35,0-7.877-3.527-7.877-7.877s3.527-7.877,7.877-7.877h210.051
      	c4.351,0,7.877,3.527,7.877,7.877S445.459,480.492,441.108,480.492z"
      />
      <path
        fill={g600}
        d="M441.108,464.738H252.062v15.754h189.046c4.351,0,7.877-3.527,7.877-7.877
      	S445.459,464.738,441.108,464.738z"
      />
    </svg>
  );
}
