import type { SVGProps } from 'react';
import { getColor } from './colors';

interface SpatulaSVGProps extends SVGProps<SVGSVGElement> {
  size?: number;
}

export default function SpatulaSVG({ size = 32, ...props }: SpatulaSVGProps) {
  const w200 = getColor('orange', 200);
  const w300 = getColor('orange', 300);
  const g100 = getColor('gray', 100);
  const g200 = getColor('gray', 200);
  const g300 = getColor('gray', 300);

  return (
    <svg
      fill="none"
      width={size}
      height={size}
      viewBox="0 0 512.001 512.001"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        fill={w200}
        d="M137.421,183.305l-39.848,39.848c-13.206,13.206-13.206,34.616-0.001,47.822l47.819,47.817
      	L9.903,454.278c-13.204,13.205-13.203,34.612,0,47.818c13.204,13.205,34.614,13.208,47.818,0L193.208,366.61l47.819,47.819
      	c13.205,13.204,34.615,13.204,47.819,0.001l39.847-39.848L137.421,183.305z M23.853,488.153c-0.002-0.003-0.003-0.004-0.006-0.006
      	c-5.49-5.493-5.488-14.432,0.003-19.923c5.492-5.493,14.43-5.495,19.923-0.002c5.495,5.495,5.496,14.431,0.006,19.925
      	C38.282,493.641,29.344,493.641,23.853,488.153z"
      />
      <path
        fill={w300}
        d="M233.058,278.943L43.776,468.226c5.492,5.495,5.492,14.429,0.003,19.923
      	c-5.497,5.492-14.435,5.492-19.926,0.006c-0.002-0.002-0.002-0.002-0.002-0.003L9.904,502.096
      	c13.205,13.205,34.614,13.205,47.817-0.001L193.208,366.61l47.819,47.819c13.205,13.204,34.615,13.204,47.819,0.001l39.847-39.848
      	L233.058,278.943z"
      />
      <polygon fill={g100} points="512.001,286.912 320.729,382.547 129.451,191.276 225.088,0 " />
      <polygon
        fill={g200}
        points="512.001,286.912 368.545,143.456 225.089,286.911 320.729,382.547 "
      />
      <path
        fill={g300}
        d="M426.324,292.89L219.11,85.676c-3.301-3.301-3.301-8.653,0.001-11.954
      	c3.301-3.302,8.653-3.302,11.954,0l207.214,207.214c3.301,3.302,3.302,8.654,0,11.955
      	C434.978,296.192,429.626,296.192,426.324,292.89z"
      />
    </svg>
  );
}
