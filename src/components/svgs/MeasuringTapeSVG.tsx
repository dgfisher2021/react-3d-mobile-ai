import type { SVGProps } from 'react';
import { getColor } from './colors';

interface MeasuringTapeSVGProps extends SVGProps<SVGSVGElement> {
  accentColor?: string;
  size?: number;
}

export default function MeasuringTapeSVG({
  accentColor = 'yellow',
  size = 32,
  ...props
}: MeasuringTapeSVGProps) {
  const ac200 = getColor(accentColor, 200);
  const g600 = getColor('gray', 600);
  const g700 = getColor('gray', 700);
  const ac100 = getColor(accentColor, 100);
  const g200 = getColor('gray', 200);
  const g300 = getColor('gray', 300);
  const g500 = getColor('gray', 500);

  return (
    <svg
      fill="none"
      width={size}
      height={size}
      viewBox="0 0 512 512"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        fill={ac200}
        d="M479.064,119.766h-62.877c-4.962,0-8.982-4.021-8.982-8.982s7.015-8.982,11.977-8.982h59.883
      	c4.962,0,8.982,4.021,8.982,8.982S484.026,119.766,479.064,119.766z"
      />
      <g>
        <path
          fill={g600}
          d="M251.509,72.196c-2.376,0-4.657-0.941-6.34-2.62c-1.691-1.685-2.642-3.974-2.642-6.363V26.947
      		c0-4.962,4.021-8.982,8.982-8.982c9.708,0,14.894,5.187,18.328,8.62c2.807,2.807,3.538,3.357,5.623,3.357
      		c2.086,0,2.818-0.55,5.624-3.358c3.432-3.431,8.618-8.618,18.328-8.618c9.709,0,14.897,5.187,18.33,8.618
      		c2.807,2.809,3.539,3.358,5.624,3.358c2.086,0,2.819-0.55,5.627-3.358c3.431-3.431,8.621-8.618,18.329-8.618
      		c4.962,0,8.982,4.021,8.982,8.982v35.93c0,4.949-4.003,8.964-8.951,8.982l-95.813,0.337
      		C251.53,72.196,251.52,72.196,251.509,72.196z"
        />
        <path
          fill={g600}
          d="M431.158,122.76c0-33.073-26.81-59.883-59.883-59.883c0,0-151.677,0-155.696,0H107.789
      		C48.26,62.877,0,111.137,0,170.667v107.789v155.696c0,33.073,26.81,59.883,59.883,59.883h155.696h107.789
      		c59.53,0,107.789-48.26,107.789-107.789V278.456c0-4.019-0.12-8.011-0.337-11.977h0.337V122.76z"
        />
      </g>
      <path
        fill={g700}
        d="M431.158,122.76c0-33.073-26.81-59.883-59.883-59.883c0,0-151.677,0-155.696,0v431.158h107.789
      	c59.53,0,107.789-48.26,107.789-107.789V278.456c0-4.019-0.12-8.011-0.337-11.977h0.337V122.76z"
      />
      <circle fill={ac100} cx="215.579" cy="278.456" r="155.696" />
      <path
        fill={ac200}
        d="M215.579,122.76v311.392c85.988,0,155.696-69.707,155.696-155.696S301.567,122.76,215.579,122.76z"
      />
      <path
        fill={g200}
        d="M239.532,194.62h-47.906c-19.843,0-35.93,16.087-35.93,35.93v71.86
      	c0,33.072,26.811,59.883,59.883,59.883s59.883-26.811,59.883-59.883v-71.86C275.462,210.707,259.375,194.62,239.532,194.62z"
      />
      <g>
        <path
          fill={g300}
          d="M239.532,194.62h-23.953v167.673c33.072,0,59.883-26.811,59.883-59.883v-71.86
      		C275.462,210.707,259.375,194.62,239.532,194.62z"
        />
        <path
          fill={g300}
          d="M503.018,131.743c-4.962,0-8.982-4.021-8.982-8.982V38.924c0-4.962,4.021-8.982,8.982-8.982
      		S512,33.962,512,38.924v83.836C512,127.722,507.979,131.743,503.018,131.743z"
        />
      </g>
      <path
        fill={g500}
        d="M215.579,311.392c-4.961,0-8.982-4.021-8.982-8.982v-59.883c0-4.962,4.022-8.982,8.982-8.982
      	s8.982,4.021,8.982,8.982v59.883C224.561,307.371,220.54,311.392,215.579,311.392z"
      />
    </svg>
  );
}
