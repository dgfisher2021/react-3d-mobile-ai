import type { SVGProps } from 'react';
import { getColor } from './colors';

interface DumpTruckSVGProps extends SVGProps<SVGSVGElement> {
  colorScheme?: string;
  accentColor?: string;
  size?: number;
}

export default function DumpTruckSVG({
  colorScheme = 'blue',
  accentColor = 'yellow',
  size = 32,
  ...props
}: DumpTruckSVGProps) {
  const g500 = getColor('gray', 500);
  const g600 = getColor('gray', 600);
  const ac300 = getColor(accentColor, 300);
  const ac400 = getColor(accentColor, 400);
  const ac200 = getColor(accentColor, 200);
  const ac100 = getColor(accentColor, 100);
  const g200 = getColor('gray', 200);
  const g100 = getColor('gray', 100);
  const cs100 = getColor(colorScheme, 100);
  const g50 = getColor('gray', 50);

  return (
    <svg
      fill="none"
      width={size}
      height={size}
      viewBox="0 0 512 512"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        fill={g500}
        d="M429.861,430.956H76.663c-4.536,0-8.214-3.678-8.214-8.214s3.678-8.214,8.214-8.214h353.198
      	c4.537,0,8.214,3.678,8.214,8.214S434.398,430.956,429.861,430.956z"
      />
      <path
        fill={g600}
        d="M435.337,414.528H251.893v16.428h183.444c4.537,0,8.214-3.678,8.214-8.214
      	S439.874,414.528,435.337,414.528z"
      />
      <polygon
        fill={ac300}
        points="62.973,203.705 62.973,159.897 139.636,159.897 205.349,203.705 369.626,203.705 
      	369.626,357.031 62.973,357.031 "
      />
      <rect x="251.893" y="203.705" fill={ac400} width="117.733" height="153.326" />
      <polygon
        fill={ac200}
        points="183.939,307.834 170.594,192.753 501.048,192.753 501.048,252.567 303.914,313.224 "
      />
      <polygon
        fill={ac300}
        points="251.893,192.753 251.893,310.886 303.914,313.224 501.048,252.567 501.048,192.753 "
      />
      <path
        fill={g600}
        d="M493.782,180.252c-13.078-12.086-13.677-6.454-30.603-19.737
      	c-8.254-6.479-15.093-17.242-25.957-24.736c-9.435-6.51-22.366-9.531-33.112-15.817c-10.355-6.055-18.865-15.61-29.884-20.617
      	c-11.329-5.147-24.586-5.46-36.029-8.616c-12.362-3.41-23.942-9.686-35.718-9.686c-9.076,0-16.499,5.83-24.077,8.63
      	c-7.998,2.955-17.619,3.548-24.066,8.684c-6.44,5.131-8.672,14.058-13.678,20.404c-5.131,6.504-13.565,11.106-17.315,17.867
      	c-4.225,7.621-3.282,16.84-5.919,23.392c-5.262,13.072-9.303,9.775-12.076,21.306"
      />
      <path
        fill={g500}
        d="M320.044,98.832c-6.439,5.131-8.669,14.058-13.677,20.404c-5.132,6.504-13.566,11.106-17.316,17.867
      	c-4.224,7.621-3.281,16.841-5.918,23.392c-5.223,12.976-9.242,9.83-12.013,21.06l-65.773,0.245
      	c2.773-11.531,6.813-8.234,12.076-21.306c2.637-6.551,1.693-15.771,5.919-23.392c3.75-6.761,12.184-11.363,17.315-17.867
      	c5.007-6.347,7.238-15.273,13.678-20.404c6.446-5.135,16.067-5.728,24.066-8.684c7.579-2.8,15.001-8.63,24.077-8.63
      	c11.778,0,23.356,6.275,35.717,9.686c0.467,0.128,0.939,0.245,1.409,0.366C332.712,93.47,325.318,94.633,320.044,98.832z"
      />
      <path
        fill={ac100}
        d="M52.021,137.994h98.567l32.856,32.856h317.604c6.049,0,10.952,4.903,10.952,10.952v10.952
      	c0,6.049-4.903,10.952-10.952,10.952H172.492l-32.856-32.856H52.021c-6.049,0-10.952-4.903-10.952-10.952v-10.952
      	C41.07,142.897,45.973,137.994,52.021,137.994z"
      />
      <circle fill={g600} cx="369.626" cy="357.031" r="65.711" />
      <circle fill={g200} cx="369.626" cy="357.031" r="21.904" />
      <circle fill={g500} cx="139.636" cy="357.031" r="65.711" />
      <circle fill={g100} cx="139.636" cy="357.031" r="21.904" />
      <path
        fill={g500}
        d="M216.299,302.272h76.663c6.049,0,10.952,4.903,10.952,10.952v43.807
      	c0,6.049-4.903,10.952-10.952,10.952h-76.663c-6.049,0-10.952-4.903-10.952-10.952v-43.807
      	C205.348,307.175,210.251,302.272,216.299,302.272z"
      />
      <rect x="19.166" y="170.849" fill={cs100} width="98.567" height="43.807" />
      <path
        fill={ac200}
        d="M19.166,203.705h120.471l10.952,87.615H73.925v65.711H8.214V214.657
      	C8.214,208.608,13.117,203.705,19.166,203.705z"
      />
      <polygon fill={g50} points="146.481,258.464 8.214,258.464 8.214,225.609 142.374,225.609 " />
      <g>
        <path
          fill={g500}
          d="M43.807,203.705v-24.642H27.38v24.642c0,4.536-3.678,8.214-8.214,8.214s-8.214-3.678-8.214-8.214
      		v-32.856c0-4.536,3.678-8.214,8.214-8.214h98.567c4.536,0,8.214,3.678,8.214,8.214v32.856c0,4.536-3.678,8.214-8.214,8.214
      		s-8.214-3.678-8.214-8.214v-24.642H93.091v24.642c0,4.536-3.678,8.214-8.214,8.214s-8.214-3.678-8.214-8.214v-24.642H60.235v24.642
      		c0,4.536-3.678,8.214-8.214,8.214S43.807,208.241,43.807,203.705z"
        />
        <path
          fill={g500}
          d="M0,258.464v98.567c0,4.536,3.678,8.214,8.214,8.214H41.07c4.536,0,8.214-3.678,8.214-8.214v-98.567
      		c0-4.536-3.678-8.214-8.214-8.214H8.214C3.678,250.25,0,253.928,0,258.464z M32.856,315.962H16.428v-16.428h16.428V315.962z
      		 M32.856,266.678v16.428H16.428v-16.428H32.856z M16.428,348.817v-16.428h16.428v16.428H16.428z"
        />
        <path
          fill={g500}
          d="M232.727,239.298H364.15c4.537,0,8.214,3.678,8.214,8.214c0,4.536-3.677,8.214-8.214,8.214H232.727
      		c-4.536,0-8.214-3.678-8.214-8.214C224.513,242.976,228.191,239.298,232.727,239.298z"
        />
      </g>
      <path
        fill={ac200}
        d="M501.048,170.849H251.893v32.856h249.155c6.049,0,10.952-4.903,10.952-10.952v-10.952
      	C512,175.752,507.097,170.849,501.048,170.849z"
      />
      <g>
        <path
          fill={g600}
          d="M292.963,302.272h-41.07v65.711h41.07c6.049,0,10.952-4.903,10.952-10.952v-43.807
      		C303.914,307.175,299.011,302.272,292.963,302.272z"
        />
        <path
          fill={g600}
          d="M364.15,239.298H251.893v16.428H364.15c4.537,0,8.214-3.678,8.214-8.214
      		C372.364,242.976,368.687,239.298,364.15,239.298z"
        />
      </g>
    </svg>
  );
}
